...
class Scholar(object):
    ...
    def __init__(self, config, alpha=1.0, learning_rate=0.001, init_embeddings=None, update_embeddings=True,
        ...
        self.n_topics = (config["num_topic"])  # change this for similarity with other implementation
    ...
    def fit(self, X, Y, PC, TC, eta_bn_prop=1.0, l1_beta=None, l1_beta_c=None, l1_beta_ci=None):
        ...
        return loss.to('cpu').detach().numpy(), Y_probs, thetas.to('cpu').detach().numpy(), nl.to(
            'cpu').detach().numpy(), kld.to('cpu').detach().numpy()
...
class torchScholar(nn.Module):
    ...
    def __init__(self, config, alpha, update_embeddings=True, init_emb=None, bg_init=None, device='cpu',
        ...
        self.vocab_size = config['num_input']  # change to be similar to other implementation
        self.words_emb_dim = config['embedding_dim']
        # self.n_topics = config['n_topics']
        self.n_topics = config['num_topic']  # change to be similar to other implementation
    ...
    def forward(self, doc_freq_vecs, doc_label_vecs, PC, TC, avg_loss=True,
                is_train=True, eta_bn_prop=1.0, var_scale=1.0, l1_beta=None, l1_beta_c=None, l1_beta_ci=None):
        """
        Do a forward pass of the model
        :param doc_freq_vecs: np.array of word counts [batch_size x vocab_size]
        :param doc_label_vecs: np.array of labels [batch_size x n_classes]
        ...
        :param is_train: if True, compute and return the loss
        :param avg_loss: if True, average the loss over the minibatch
        ...
        en0_x = self.embeddings_x_layer(doc_freq_vecs)
        encoder_parts = [en0_x]
        ...
        if self.n_labels > 0:
            encoder_parts.append(doc_label_vecs)
        ...
        # print(posterior_logvar_bn.max().item(), posterior_logvar.mean().item(), posterior_logvar.var().item(),
        #      posterior_logvar.max().item())

        posterior_var = posterior_logvar_bn.exp().to(self.device)

        if is_train:
            # sample noise from a standard normal
            eps = doc_freq_vecs.data.new().resize_as_(posterior_mean_bn.data).normal_().to(self.device)
            ...
            if self.n_topic_covars > 0:
                ...
                if self.use_interactions:
                    ...
                    eta += self.beta_ci_layer(
                        covar_interactions.reshape((batch_size, self.n_topics * self.n_topic_covars)))
            ...
            return self._loss(doc_freq_vecs, doc_label_vecs, X_recon, Y_recon, prior_mean, prior_logvar,
                              posterior_mean_bn, posterior_logvar_bn, avg_loss, l1_beta, l1_beta_c, l1_beta_ci)
        else:
            # pass the document representations through a softmax
            theta = F.softmax(posterior_mean_bn, dim=1)
            ...
            return posterior_mean_bn, theta, X_recon, Y_recon, self._loss(doc_freq_vecs, doc_label_vecs, X_recon,
                                                                          Y_recon, prior_mean, prior_logvar,
                                                                          posterior_mean_bn, posterior_logvar_bn,
                                                                          avg_loss)

    def _loss(self, X, Y, X_recon, Y_recon, prior_mean, prior_logvar, posterior_mean, posterior_logvar, avg_loss=True,
              l1_beta=None, l1_beta_c=None, l1_beta_ci=None):
        # print('self.l1_beta_reg = {}, l1_beta = {}, l1_beta_c = {}, l1_beta_ci = {}'.format(self.l1_beta_reg, l1_beta,
        ...
        # print(var_division.sum(1).mean().item(), diff_term.sum(1).mean().item(), logvar_division.sum(1).mean().item(),
        #      NL.mean().item(), KLD.mean().item(), posterior_logvar.max().item())
        if avg_loss:
            return loss.mean(), NL.mean(), KLD.mean()
        else:
            # print(loss.mean(), loss.min(), loss.max(), loss.median())
            return loss, NL, KLD
